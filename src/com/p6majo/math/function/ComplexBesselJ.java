 ...
 *
 */
public class ComplexBesselJ extends ComplexFunction{
	
		private int order;
		
		public ComplexBesselJ(int order){
			this.order = order;
		}
		...
		public Complex eval(Complex z) {
			...
			int steps = 2;
			//values at the boundary of the integration interval
			Complex[] f= new Complex[2];
			f[0] = Complex.ONE;
			if (this.order%2==0) f[1]=Complex.ONE; else f[1]=Complex.ONE.neg();
			...
			while (Inext.minus(Istart).divides(Inext).abs()>Complex.EPS_FLOAT || k==1) {
				...
				for (int i=0;i<steps;i++) {
					...
					if (i%2==0) fnext[i]=f[ih]; 
						...
						fnext[i] = z.times(Complex.ONE.scale(Math.sin(x))).minus(Complex.ONE.scale(this.order*x)).cos(); 
					}	
				}
		...
		public Complex evalDerivative(Complex z) {
			if (this.order==0) {
				ComplexBesselJ besselj=new ComplexBesselJ(1);
				return besselj.eval(z).neg();
			}
			else{
				ComplexBesselJ besseljminus = new ComplexBesselJ(this.order-1);
				ComplexBesselJ besseljplus = new ComplexBesselJ(this.order+1);
				
				
				
				return (besseljminus.eval(z).minus(besseljplus.eval(z))).scale(0.5);
			}
		}
