...
import static com.nr.NRUtil.SIGN;
...
public class ComplexOdeint {
   ...
   */
  ComplexDerivativeInf derivs;
  ComplexStepperBase s;
  int nstp;
  double h;

  /* XXX ref to StepperBase
  double[] y,dydx;
  double x;
  */
  /**
   ...
   * @param s
   */
  public ComplexOdeint(final Complex[] ystartt, final double xx1, final double xx2,
                       final double atol, final double rtol, final double h1,
                       final double hminn, final ComplexOutput outt, final ComplexDerivativeInf derivss, final ComplexStepperBase s) {
    nvar = ystartt.length;
    ystart = ystartt;
    Complex[] y = new Complex[nvar];
    Complex[] dydx = new Complex[nvar];
    double x =xx1;
    nok =0;
    nbad=0;
    x1=xx1;
  ...
  public void integrate() {
    ...
    for (nstp=0;nstp<MAXSTP;nstp++) {
      ...
      if (dense)
        ...
        out.save(s.x,s.y);
      if ((s.x-x2)*(x2-x1) >= 0.0) {
        for (int i=0;i<nvar;i++) ystart[i]=s.y[i];
        if (out.kmax > 0 && abs(out.xsave[out.count-1]-x2) > 100.0*abs(x2)*EPS)
          out.save(s.x,s.y);
        return;
      }
      if (abs(s.hnext) <= hmin) throw new IllegalArgumentException("Step size too small in Odeint");
      h=s.hnext;
    }
  ...
  /*
  public static void rk4(final double[] y, final double[] dydx, final double x, final double h, final double[] yout, final DerivativeInf derivs) {
    int n=y.length;
