...
public class Qotree {
  ...
  /**
   * Constructor for a quad- (DIM=2) or oc- (DIM=3) tree that can store a max of
   * nv elements of type elT, using hash tables of length nh (typically nv).
   * maxdep is the number of levels to be represented.
   * 
   * @param dim
   * @param nh
   * @param nv
  ...
  public Qotree(final int dim, final int nh, final int nv, final int maxdep) {
    ...
    if (maxd > PMAX) throw new IllegalArgumentException("maxdep too large in Qotree");
    setouterbox(new Point(DIM, new double[]{0.0,0.0,0.0}), new Point(DIM, new double[]{1.0,1.0,1.0}));
  }
  ...
  public Box qobox(final int kk) {
    ...
    while (k > 1) {
      kb = (k + QL) % QO;
      for (j=0; j<DIM; j++) { if ((kb & (1 << j))!=0) offset[j] += del; }
      k = (k + QL) >> DIM;
      del *= 2.0;
    }
    for (j=0; j<DIM; j++) {
  ...
  /**
   * Return the box number of the smallest box that can contain an element tobj,
   * without regard to whether tobj is already stored in the tree.
   * 
   * @param tobj
   * @return
   */
  public int qowhichbox(final Sphcirc tobj) {
    ...
    for (p=2; p<=maxd; p++) {
      ...
      kr = kl + QO -1;
      for (k=kl; k<=kr; k++) {
        if (tobj.isinbox(qobox(k))!=0) { ks = k; break; }
      }
      if (k > kr) break;
    }
    return ks;
  ...
  /**
   * Store the element tobj in the Qotree, and return the box number into which
   * it was stored.
   *
   * @return
   */
   ...
   * or 0 if the element was not found in the Qotree. Note logic very similar to
   * qostore.
   * 
   * @param tobj
   * @return
  ...
  public int qoerase(final Sphcirc tobj) {
    ...
    if (elhash.count(ks)!=0) return kks;
    ppop = pophash.get(ks);
    if(ppop==null)
      ppop=0;
    pophash.set(ks, ppop&~(1));
    while (ks > 1) {
      if (pophash.get(ks)!=0) break;
      pophash.erase(ks);
      km = (ks + QL) >> DIM;
      k = ks - (QO*km - QL);
      ks = km;
   ...
   * box k of the Qotree. The elements are copied into list[0..nlist-1] and the
  ...
  public int qoget(final int k, final Sphcirc[] list, final int nmax) {
    ...
    if ((pop & 1)!=0 && elhash.getinit(ks)!=0) {
      while (nlist < nmax && elhash.getnext(list,nlist)!=0) {nlist++;}
    }
    return nlist;
  }
  ...
  public int qodump(final int[] klist, final Sphcirc[] list, final int nmax) {
    ...
    while (ntask!=0) {
      ks = tasklist[ntask--];
      Integer[] pop_w = new Integer[1];
      if (pophash.get(ks,pop_w,0) == 0) continue;
      pop=pop_w[0];
      if ((pop & 1)!=0 && elhash.getinit(ks)!=0) {
   ...
   * contain the point pt. The elements are copied into list[0..nlist-1] and the
   * value nlist (<= nmax) is returned.
   * 
   * @param pt
   * @param list
    ...
    while (pophash.get(ks,pop_w,0)!=0) {
      ...
      if ((pop & 1)!=0) {
        ...
        while (nlist < nmax && elhash.getnext(list,nlist)!=0) {
          ...
          if (list[nlist].contains(pt)!=0) {nlist++;}
        }
      }
      if ((pop >>= 1) == 0) break;
      for (k=0, j=0; j<DIM; j++) {
  ...
  public int qocollides(final Sphcirc qt, final Sphcirc[] list, final int nmax) {
    ...
    while (ntask!=0) {
      ...
      if ((pop & 1)!=0) {
        elhash.getinit(ks);
        while (nlist < nmax && elhash.getnext(list,nlist)!=0) {
          if (list[nlist].collides(qt)!=0) {nlist++;}
        }
      }
      if (ks >= kks) {
        k = QO*ks - QL;
