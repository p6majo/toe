...
import static com.nr.root.Roots.zriddr;
...
public class Test_zriddr {
  ...
  public void test() {
    ...
    System.out.printf("zriddr: Maximum discrepancy = %f\n", maxel(vecsub(root,yy)));
    localflag = maxel(vecsub(root,yy)) > xacc;
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** zriddr: Incorrect roots");
  ...
  class Func_zriddr implements UniVarRealValueFun{
    Bessjy b = new Bessjy();
