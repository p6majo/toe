...
public class ConvolutionWrapper {
    ...
    public INDArray convolution(INDArray image, INDArray kernel){
        ...
        convolutionwrap(imagef,rowsImg,colsImg,kernelf,rowsKer,rowsImg,resultf);

        INDArray result =Nd4j.create(new int[]{rowsResult,colsResult},resultf);
        return result;
    }

    public native void convolutionwrap(float[] image, int rowsImg, int colsImg, float[] kernel, int rowsKer, int colsKer, float[] convolution);
    static{
