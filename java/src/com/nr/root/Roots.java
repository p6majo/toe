...
public class Roots {
  ...
  public static boolean zbrac(final UniVarRealValueFun func,
    ...
    if (x1 == x2) throw new IllegalArgumentException("Bad initial range in zbrac");
  ...
  public static class Zbrak {
    ...
    public void zbrak(final UniVarRealValueFun fx, final double x1, final double x2, final int n) {
      int nb = 20;
      xb1 = new double[nb];
      xb2 = new double[nb];
      nroot=0;
   ...
   * @return
   */
  public static double rtbis(final UniVarRealValueFun func, final double x1, final double x2, final double xacc) {
    final int JMAX=50;
    double dx,xmid,rtb;
    double f=func.funk(x1);
    double fmid=func.funk(x2);
    if (f*fmid >= 0.0) throw new IllegalArgumentException("Root must be bracketed for bisection in rtbis");
   ...
   * func known to lie between x1 and x2. The root is refined until its accuracy
   * is +/-acc
   * 
   * @param func
   * @param x1
   ...
   * @return
   */
  public static double rtflsp(final UniVarRealValueFun func, final double x1, final double x2, final double xacc) {
    final int MAXIT=30;
    double xl,xh,del;
    double fl=func.funk(x1);
    double fh=func.funk(x2);
    if (fl*fh > 0.0) throw new IllegalArgumentException("Root must be bracketed in rtflsp");
    if (fl < 0.0) {
      xl=x1;
      xh=x2;
    } else {
    ...
    throw new IllegalArgumentException("Maximum number of iterations exceeded in rtflsp");
  }
  
  /**
   * Using the secant method, return the root of a function or functor func
   * thought to lie between x1 and x2. The root is refined until its accuracy is
   * +/-acc.
   * 
   * @param func
   * @param x1
   ...
   * @return
   */
  public static double rtsec(final UniVarRealValueFun func, final double x1, final double x2, final double xacc) {
    final int MAXIT=30;
    double xl,rts;
    double fl=func.funk(x1);
    ...
    throw new IllegalArgumentException("Maximum number of iterations exceeded in rtsec");
  }
  
  /**
   * Using Ridders' method, return the root of a function or functor func known
   ...
   * @return
   */
  public static double zriddr(final UniVarRealValueFun func, final double x1, final double x2, final double xacc) {
    final int MAXIT=60;
    double fl=func.funk(x1);
    double fh=func.funk(x2);
    if ((fl > 0.0 && fh < 0.0) || (fl < 0.0 && fh > 0.0)) {
      ...
      for (int j=0;j<MAXIT;j++) {
        ...
        if (SIGN(fm,fnew) != fm) {
          ...
          xl=ans;
          fl=fnew;
        } else throw new IllegalArgumentException("never get here.");
        if (abs(xh-xl) <= xacc) return ans;
      }
      throw new IllegalArgumentException("zriddr exceed maximum iterations");
    }
    else {
      if (fl == 0.0) return x1;
      if (fh == 0.0) return x2;
      throw new IllegalArgumentException("root must be bracketed in zriddr.");
    }
  }
  
  /**
   * Using Brent's method, return the root of a function or functor func known
   * to lie between x1 and x2. The root will be refined until its accuracy is tol.
   * 
   * @param func
   * @param x1
   * @param x2
   ...
   * @return
   */
  public static double zbrent(final UniVarRealValueFun func, final double x1, final double x2, final double tol) {
    final int ITMAX=100;
    final double EPS=DBL_EPSILON;
    double a=x1,b=x2,c=x2,d=0,e=0,fa=func.funk(a),fb=func.funk(b),fc,p,q,r,s,tol1,xm;
  ...
  /**
   * Using the Newton-Raphson method, return the root of a function known to lie
   * in the interval [x1, x2]. The root will be refined until its accuracy is
   * known within +/-acc. funcd is a user- supplied struct that returns the
   * function value as a functor and the first derivative of the function at the
   ...
   * @return
   */
  public static double rtnewt(final UniValRealValueFunWithDiff funcd, final double x1, final double x2, final double xacc) {
    final int JMAX=20;
    ...
    for (int j=0;j<JMAX;j++) {
      ...
      if ((x1-rtn)*(rtn-x2) < 0.0)
        throw new IllegalArgumentException("Jumped out of brackets in rtnewt");
      if (abs(dx) < xacc) return rtn;
    }
    throw new IllegalArgumentException("Maximum number of iterations exceeded in rtnewt");
  }
   ...
   * @return
   */
  public static double rtsafe(final UniValRealValueFunWithDiff funcd, final double x1, final double x2, final double xacc) {
    final int MAXIT=100;
    double xh,xl;
    ...
    throw new IllegalArgumentException("Maximum number of iterations exceeded in rtsafe");
  }
   ...
   * @param eps
   */
  public static void qroot(final double[] p, final doubleW b, final doubleW c, final double eps) {
    final int ITMAX=20;
    ...
    throw new IllegalArgumentException("Too many iterations in routine qroot");
  }
  
  /**
   * polynomial roots by eigenvalue method
   * 
   * Find all the roots of a polynomial with real coefficients, S(i=0,m)a[i]x^i,
   * given the coefficients a[0..m]. The method is to construct an upper
   * Hessenberg matrix whose eigenvalues are the desired roots and then use the
   * routine Unsymmeig. The roots are returned in the complex vector rt[0..m-1],
   * sorted in descending order by their real parts.
   * 
   * @param a
   * @param rt
   */
  ...
  private static final double[] frac= {0.0,0.5,0.25,0.75,0.13,0.38,0.62,0.88,1.0};
   ...
   * complex roots in roots[0..m-1]. The boolean variable polish should be input
   * as true if polishing (also by Laguerre's method) is desired, false if the
   ...
   * point x[0..n-1] along the direction p from xold where the function or
   ...
   * is undefined or subject to overflow. p is usually the Newton direction. The
   * output quantity check is false on a normal exit. It is true when x is too
   * close to xold. In a minimization algorithm, this usually signals
   ...
   * @param func
   */
  public static void lnsrch(final double[] xold, final double fold, final double[] g, final double[] p,
      final double[] x, final doubleW f, final double stpmax, final booleanW check, final RealValueFun func) {
    ...
    for (i=0;i<n;i++)
      slope += g[i]*p[i];
    if (slope >= 0.0) throw new IllegalArgumentException("Roundoff problem in lnsrch.");
    test=0.0;
    for (i=0;i<n;i++) {
    ...
    for (;;) {
      ...
      if (alam < alamin) {
        ...
        check.val=true;
        return;
      } else if (f.val <= fold+ALF*alam*slope) return;
      else {
        if (alam == 1.0)
   ...
   * zeroed, called fvec[0..n-1] in the routine below, is returned by the
   * user-supplied function or functor vecfunc (see text). The output quantity
   * check is false on a normal return and true if the routine has converged to
   * a local minimum of the function fmin defined below. In this case try
   ...
   * @param vecfunc
   */
  public static void newt(final double[] x, final booleanW check, final RealMultiValueFun vecfunc) {
  ...
  /**
   * Given an initial guess x[0..n-1] for a root in n dimensions, find the root
   * by Broyden's method embedded in a globally convergent strategy. The vector
   * of functions to be zeroed, called fvec[0..n-1] in the routine below, is
   * returned by the user-supplied function or functor vecfunc. The routines
   * NRfdjac and NRfmin from newt are used. The output quantity check is false
   * on a normal return and true if the routine has converged to a local minimum
   ...
   * @param vecfunc
   */
  public static void broydn(final double[] x, final booleanW check, final RealMultiValueFun vecfunc) {
    final int MAXITS=200;
    final double EPS=DBL_EPSILON;
