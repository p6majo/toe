...
public class StepperBS extends StepperBase{
  ...
  public void dy(final double[] y,final double htot,final int k,final double[] yend,
    final intW ipt,final DerivativeInf derivs) {
  ...
  public void prepare_dense(final double h,final double[] dydxnew,
    final double[] ysav,final double[] scale,final int k,final doubleW error) {
    mu=2*k-1;
    for (int i=0; i<n; i++) {
      dens[i]=ysav[i];
      dens[n+i]=h*dydx[i];
